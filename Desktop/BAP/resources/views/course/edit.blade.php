@extends('layouts.app')
    @section('title','TattooEase | Cursus Aanpassen')
@push('scripts')
...
<div id="url-create-route-button">
    <p><a href="{{route('dashbord', auth()->user()->id)}}">Dashboard</a> <span>/</span> <a href="{{route('course-overview')}}">Cursussen</a> <span>/</span> <a href="{{route('course-detail', $course->id)}}"> {{$course->title}} </a><span>/</span> {{$course->title}} Aanpassen</p>
</div>
{{-- Datepicker modal --}}
...
<div id="crud-form-div">
    ...
    <div id="course-edit-div" style="display:none;"></div>
    {{-- Formulier --}}
    <div id="form-div">
        <form action="{{route('course-edit-submit', $course->id)}}" method="POST" enctype="multipart/form-data">
            {{ method_field('PATCH') }}
            @if (session('fail'))
            ...
            {{-- <div class="form-item clearfix" id="date-div">
                <div>
                    ...
                    <input type="text" value="{{$course->from_date}}" class="datepicker-input" readonly name="from" id="from" required autocomplete="off">     
                </div>
                <div>
                    <label for="until" data-for="until" >Tot <span id="untilSpan" style="margin-left:5px;"></span></label>
                    <input type="text" value="{{$course->until_date}}" class="datepicker-input" readonly name="until" id="until" required autocomplete="off">
